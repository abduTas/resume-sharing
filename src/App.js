...
class App extends Component {
    ...
    getUser() {
      ...
      userResumeDetails(resumeObj) {
          this.setState({resumeObj})
      }
  render() {
    let publicpath =   !!window.location.href.includes("public") && `/public/view/${ window.location.href.substring(window.location.href.lastIndexOf('/')+1)}`
    console.log("object", publicpath)
    return (
      <div className="App">
      ...
      {/* <Route
        exact path="/" render={() =>
            <Home 
            updateUser={this.userResumeDetails}
            />} /> */}
        <PrivateRoute exact  path="/" username={this.state.username} updateUser={this.userResumeDetails} component={Home} />
