...
var storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    let filename =  req.user.username+ '.'+file.mimetype.slice(file.mimetype.lastIndexOf('/')+1)
    console.log("multeruser1", filename)

    User.findOneAndUpdate({username: req.user.username} , {$set:{imageUrl :  filename}},{new:true , upsert:true},(err,res)=>{
        if(err){
            console.log(err)
        }
        console.log(res)
...
router.put('/uploadDetails',(req,res)=>{
    let data = req.body
    debugger;
    User.findOneAndUpdate({username: req.user.username} , {$set:{...req.body}},{new:true },(err,res)=>{
        if(err){
            console.log(err)
        }
        console.log(res)
...
router.get('/userdetails', (req, res, next) => {
    ...
    if (req.user) {
        User.findOne({username: req.user.username},(err,response)=>{
            ...
            // console.log(res)
            res.status(200).send({userdetails:response})
        })
    } else {
        res.json({user: null})
    }
...
router.get('/userdetails/public/:username', (req, res, next) => {
